...
- (void)storePropertiesToArchive
{
    ...
    if (profileCount == 0)
    {
        ...
        IoTProfile *profile = [[IoTProfile alloc] initWithName:@"default" routerAddress:self.routerAddress userName:self.userName password:self.password];
    ...
    [dataDict setObject:[self.currentProfile profileName] forKey:@"cardemo:selectedprofile"];
...
- (void)loadProfilesFromArchive
{
    ...
    if (data != nil)
    {
        ...
        if ((profileName = [data objectForKey:@"cardemo:selectedprofile"]) == nil)
        {
            profileName = @"";
        }
        
        if ([data objectForKey:IOTUserName] != nil)
        {
            // Data from older app version -- no profile
            NSString *userName = [data objectForKey:IOTUserName];
            NSString *routerAddress = [data objectForKey:IOTRouterAddress];
            NSString *password = [data objectForKey:IOTPassword];
            self.currentProfile = [[IoTProfile alloc] initWithName:@"default" routerAddress:routerAddress userName:userName password:password];
            [self.profiles addObject:self.currentProfile];
        ...
        else
        {
            ...
            while ((key = [keys nextObject]) != nil)
            {
                ...
                if ([key isEqualToString:@"cardemo:selectedprofile"])
                {
                    continue;
                }
        ...
        {
            self.password = [self.currentProfile password];
            self.routerAddress = [self.currentProfile routerAddress];
            self.userName = [self.currentProfile userName];
        }
    }
}
...
/** Publish accelerometer data.  Not needed for controller, but use for Sensor
 */
    ...
    NSString *messageData = [MessageFactory createAccelMessage:self.latestAcceleration.x
                                                       ...
                                                       accel_z:self.latestAcceleration.z
                                                         ...
                                                         pitch:self.latestPitch
                                                           yaw:self.latestYaw
                                                           lat:self.latestLocation.latitude
                                                           lon:self.latestLocation.longitude];
    
    [self.carDemoControllerViewController updateAccelLabels];
    [self publishData:messageData event:IOTAccelEvent VIN:self.VIN];
}
 ...
 */
- (void)publishData:(NSString *)message event:(NSString *)event VIN:(NSString *)VIN
{
    NSLog(@"%s:%d entered", __func__, __LINE__);
    ...
    [messenger publish:[TopicFactory getEventTopic:event VIN:VIN] payload:message qos:0 retained:NO];
    self.publishCount++;
    [self.carDemoControllerViewController updateMessageCounts];
}
/** Publish an MQTT Message with data message for IoT Event event
 *  @param message The data to be sent
 *  @param event The event to publish the data to
...
- (void)publishCommand:(NSString *)message command:(NSString *)command QoS:(int)QoS VIN:(NSString *)VIN
{
    NSLog(@"%s:%d entered", __func__, __LINE__);
    Messenger *messenger = [Messenger sharedMessenger];
    if (messenger.client.isConnected == NO)
    {
        NSLog(@"Mqtt Client not connected. Commands will not be sent");
        return;
    }
    
    // Publish the message on the desired event
//    [messenger publish:[TopicFactory getCommandTopic:event] payload:message qos:0 retained:NO];
    [messenger publish:[TopicFactory getCommandTopic:command VIN:VIN] payload:message qos:QoS retained:NO];
    self.publishCount++;
    [self.carDemoControllerViewController updateMessageCounts];
}
...
- (void)updateViewLabelsAndButtons
{
    ...
    if (self.carDemoControllerViewController != nil) {
        [self.carDemoControllerViewController updateViewLabels];
    }
}
...
- (void)updateColor:(UIColor *)color
{
    NSLog(@"%s:%d entered", __func__, __LINE__);
    self.color = color;
    [self.carDemoControllerViewController updateBackgroundColor:self.color];
}
...
- (void)loadStoryboard
{
    //dropping all the other storyboards
    UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Second_iPhone" bundle:nil];
...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation {
    self.latestLocation = CLLocationCoordinate2DMake(newLocation.coordinate.latitude , newLocation.coordinate.longitude ) ;
}
