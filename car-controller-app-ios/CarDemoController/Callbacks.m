     ...
     errorMessage:(NSString *)errorMessage
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        NSString *message = [NSString stringWithFormat:@"Failed to connect to Solace Router. Reason Code: %d", errorCode];
...
{
    // topicParts: @"cardemo/vin/cmd/%@/fmt/%@"
    //   [0] = cardemo
    //   [1] = vin
    //   [2] = cmd
    //   [3] = <command>
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [appDelegate.carDemoControllerViewController updateMessageCounts];
        
        if ([topicParts[3] isEqualToString:IOTColorEvent])
        {
            [self processColorMessage:payload];
        }
        else if ([topicParts[3] isEqualToString:IOTTextEvent])
        {
            [self processTextMessage:payload];
        }
        else if ([topicParts[3] isEqualToString:IOTLightEvent])
        {
            [appDelegate toggleLight];
        } else if ([topicParts[3] isEqualToString:IOTAlertEvent])
        {
            [self processAlertMessage:payload];
        } else if ([topicParts[3] isEqualToString:IOTCrashEvent])
        {
            [self processCrashOrTrackMessage:payload isCrash:YES];
        } else if ([topicParts[3] isEqualToString:IOTTrackingEvent])
        {
            [self processCrashOrTrackMessage:payload isCrash:NO];
        }
    });
}
...
/** Process a crash or tracking event message.
 *  @param payload The message payload
 */
- (void)processCrashOrTrackMessage:(NSString *)payload isCrash:(BOOL)isCrash
{
    AppDelegate *appDelegate = [[UIApplication sharedApplication] delegate];
    NSDictionary *body;
    body = [self getMessageBody:payload];
    ...
    NSNumber *accel_x = [body objectForKey:JSON_ACCEL_X];
    NSNumber *accel_y = [body objectForKey:JSON_ACCEL_Y];
    NSNumber *accel_z = [body objectForKey:JSON_ACCEL_Z];

    appDelegate.latestLocation = CLLocationCoordinate2DMake([[body objectForKey:JSON_LAT] doubleValue], [[body objectForKey:JSON_LON] doubleValue] );
    if (appDelegate.trackerViewController != nil) {
        [appDelegate.trackerViewController updatePinLocation];
    }
    
    if (isCrash) {
        appDelegate.hasCrashed = YES;
        CMAcceleration accel;
        accel.x = [accel_x doubleValue];
        accel.y = [accel_y doubleValue];
        accel.z = [accel_z doubleValue];
        [appDelegate updateAccelerationData:accel];
        [appDelegate.carDemoControllerViewController updateAccelLabels];
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Crash Detected!" message:@"Switching to Map to show Crash location." delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
        [alert setAlertViewStyle:UIAlertViewStyleDefault];
        [alert show];
        [appDelegate.carDemoControllerViewController playCrashSound];
        [appDelegate switchToTrackingView];
    }
//    [appDelegate addLogMessage:<values>];
}
