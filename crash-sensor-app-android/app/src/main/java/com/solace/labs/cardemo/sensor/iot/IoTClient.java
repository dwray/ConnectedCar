...
public class IoTClient {
    ...
    private IoTClient(Context context, String organization, String deviceID, String deviceType, String authorizationToken, String gLevel, String VIN) {
        this.context = context;
        this.client = null;
     ...
     * @param gLevel Used to send an SMS
     *
     * @return The IoTClient object for the application
     */
    public static IoTClient getInstance(Context context, String organization, String deviceID, String deviceType, String authorizationToken, String gLevel, String vin) {
        Log.d(TAG, ".getInstance() entered");
        if (instance == null) {
            instance = new IoTClient(context, organization, deviceID, deviceType, authorizationToken, gLevel, vin);
        } else {
            instance.setAuthorizationToken(authorizationToken);
            instance.setOrganization(organization);
            instance.setDeviceID(deviceID);
            instance.setDeviceType(deviceType);
            instance.setgLevel(gLevel);
            instance.setVIN(vin);
        }
        return instance;
    }
    ...
    public IMqttToken connectDevice(IoTCallbacks callbacks, IoTActionListener listener, SocketFactory factory) throws MqttException {
        ...
        String clientID = "d:" + this.getOrganization() + ":" + this.getDeviceType() + ":" + getVIN() + "_sensor";
...
//DWDW maybe make some changes here
        if (!isMqttConnected()) {
            ...
            client = new MqttAndroidClient(context, connectionURI, clientID, MqttAndroidClient.Ack.AUTO_ACK);
 ...
 //           String username = IOT_DEVICE_USERNAME;
            ...
            // we need to keep session for colour messages
            options.setCleanSession(false);
            options.setUserName(username);
            options.setPassword(password);
            options.setKeepAliveInterval(10);
            options.setMaxInflight(255);
    ...
    /**
     * Disconnect the device from the Solace Router
     *
     * @param listener  The IoTActionListener object to register with the Mqtt Token.
     *
    ...
    public static String getEventTopic(String event, String format) {
        if (format.contentEquals("json")) {
            return "cardemo/"+getVIN()+"/evt/" + event + "/fmt/json";
        } else if (format.contentEquals("text")){
            return "cardemo/"+getVIN()+"/evt/" + event + "/fmt/text";
        } else {
            return "cardemo/"+getVIN()+"/evt/" + event + "/fmt/unkw";
        }
    }
    ...
    public static String getCommandTopic(String command, String format) {
        return "cardemo/"+getVIN()+"/cmd/" + command + "/fmt/json";
    }
